function sym_DKL = ComputeJSD(bamp1, bamp2)
...
%symmetrical KL distance instead of the 


vec1 = bamp1./sum(bamp1);
vec2 = bamp2./sum(bamp2);

% DKL = kldiv(X, vec1, vec2, 'sym');
...
vec3 = (vec1+vec2)./2;
KL1 = sum(vec1.* (log(vec1)-log(vec3)));
KL2 = sum(vec2.* (log(vec2)-log(vec3)));
JSD = (KL1+KL2)/2;
sym_DKL = JSD;
end